...
def run_tests(test_type="all", verbose=True, coverage=False, parallel=False):
    ...
    if coverage:
        cmd.extend(["--cov=cif2qewan", "--cov-report=html", "--cov-report=term-missing"])
    ...
    cmd.extend(["-m", "not requires_qe and not requires_wannier90 and not requires_cif2cell and not requires_api"])
